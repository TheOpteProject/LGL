						  ...
						  MouseMotionListener
{
    ...
    // CONSTRUCTOR
    public EdgesPanel( Edge[] edges , Vertex[] vertices , 
	...
	for (int ii=0; ii<edges.length; ++ii ) 
	    {	    
		Matrix v1 = edges[ii].vertex1().location();
		Matrix v2 = edges[ii].vertex2().location();
	    
		if ( doVisibilityTest )
		    if ( ! visible(v1,v2) ) 
			continue;
		
		Object o = edgeColorMap.get( edges[ii] );
		
		if ( o == null )
		    {
			g2.draw( new Line2D.Double( v1.get(0,0),
						    v1.get(1,0),
						    v2.get(0,0),
						    v2.get(1,0) ) );
		    }
	...
	while ( i.hasNext() )
		...
		Matrix v1 = e.vertex1().location();
		Matrix v2 = e.vertex2().location();
	...
	for ( int ii=0; ii<vertices.length; ++ii )
		...
		Object o = vertexColorMap.get( vertices[ii] );
		if ( o != null )
		    continue;

		Matrix m = vertices[ii].location();
		g2.fill( new Rectangle( (int)m.get(0,0)-vertexRadius , 
					(int)m.get(1,0)-vertexRadius , 
    ...
    public void fitData() { formatter.fitDataToWindow(); }    
    ...
    public void addEdgeColors ( HashMap h ) { edgeColorMap.putAll( h ); }
    public void addVertexColors( HashMap h ) { vertexColorMap.putAll( h ); }
    public void clearAllEdgeColors() { edgeColorMap.clear(); }
    public void clearAllVertexColors() { vertexColorMap.clear(); }
    public void setEdgeIdMap( HashMap h ) { edgeIdMap = h; }
    public void setVertexIdMap( HashMap h ) { vertexIdMap = h; }
    public void setVertexRadius( int r ) { vertexRadius = r; }
    public void setVertices( Vertex[] v ) { vertices = v; }
    public void setFormatter( FormatVertex v ) { formatter = v; }
    public void setFontColor( Color c ) { fontColor=c; }
    public void setEdgeColor( Color c ) { edgeColor=c; }
    public void setVertexColor( Color c ) { vertexColor=c; }
    public void setFont( Font s ) { font = s; }
    public void setMoveStepSize( double s ) { moveStepSize=s; }
    public void setBackgroundColor( Color c ) { backgroundColor=c; }
    public void setMins( Matrix m ) { mins = m; }
    public void setMaxs( Matrix m ) { maxs = m; }
    public void prepZoomRegion( boolean s ) { zoomRegion = s; }
    public void prepZoomPoint( boolean s ) { zoomPoint = s; }
    public void prepIdRegion( boolean s ) { idRegion = s; }
    public void setZoomStepSize( double s ) { zoomStepSize = s; }
    public void setVisibilityTest( boolean b ) { doVisibilityTest=b; }

    public void setStatusBar( JTextField f )
    { 
	statusBar = f; statusBar.setText( statusMessage );
    }
    
    // ACCESSORS
    public int windowSizeX() { return xWindowSize; }
    public int windowSizeY() { return yWindowSize; }
    public Edge[] getEdges() { return edges; }
    public Vertex[] getVertices() { return vertices; }
    public HashMap getEdgeColors() { return edgeColorMap; }
    public HashMap getVertexColors() { return vertexColorMap; }
    public double getZoomStepSize() { return zoomStepSize; }
	...
	try {
	    ...
	    JOptionPane.showMessageDialog( null , "Image Write Failed" , 
					   "Error" , JOptionPane.ERROR_MESSAGE );
	}
	if ( statusBar != null ) { statusBar.setText( "Done." ); }
    }

    // Produce an image of a given region
    public void imageRegion( Matrix mins , Matrix maxs , String imageName , BufferedImage i )
    {
	...
	while ( i.hasNext() )
		...
		Color c = (Color) entry.getValue();
		float[] rgb = c.getColorComponents( null );
		buffer += id1 + " " + id2 + " " + rgb[0] + " " + rgb[1] + " " + rgb[2] + "\n";
	    }
	try {
	    FileWriter out = new FileWriter( f );
	    out.write( buffer );
	...
	for ( int ii=0; ii<vertices.length; ++ii )
		...
		if ( vertices[ii].doesShowID() || idsIncluded )
			...
			g.drawString( vertices[ii].id() , 
				      (int) vertices[ii].location().get(0,0) ,
				      (int) vertices[ii].location().get(1,0) ) ;
		    }
	    }
    }
	...
	if ( highlightSanityCheck() )
		...
		for ( int ii=0; ii<vertices.length; ++ii )
			...
			if ( ((mins.get(0,0) < x.get(0,0)) &&
			     (x.get(0,0) < maxs.get(0,0))) &&
			     ((mins.get(1,0) < x.get(1,0)) &&
			     (x.get(1,0) < maxs.get(1,0))) ) {
	...
	if ( f.willLabel() || f.willZoom() )
		...
		setIncomingLabelsAndStats( ids , stats , 
					   f.willLabel() , f.willZoom() );
		statusBar.setText("Done.");
	    }
	...
	if ( f.willColorEdges() )
	    {
		setEdgeColorsFromFindVertexFrame( ids , 
						  f.getEdgeColor() );
	    }
	...
	for( int ii=0; ii<ids.length; ii += 2 )
		...
		if ( o == null ) { handleBadEdge(id1,id2); continue; }
		h.put( (Edge)o , color );
		//System.out.println("COLOR: " + color);
		//edges[jj].print();
	    }
    ...
    {
	JOptionPane.showMessageDialog( null , 
				       id1 + " and " + id2 + " is an undefined Edge." ,
				       "Error" , JOptionPane.ERROR_MESSAGE );
    }
	...
	if ( scales[0] > 1 ) { moves[0] *= -1; }
	moves[1] = zoomStepSize * yWindowSize;
	if ( scales[1] > 1 ) { moves[1] *= -1; }
	...
	if ( zoomRegion || idRegion ) {
	    x2 = e.getX();
	    y2 = e.getY();
	    statusBar.setText( "Region set to (" + x1 + "," + y1 + 
			       ") to (" + e.getX() + "," + e.getY() +
			       ") Working on redraw..." );
    ...
    { 
	if ( zoomRegion || idRegion ) {
	    statusBar.setText("Highlighting region from (" +
			      x1 + "," + y1 + ") to (" + e.getX() + "," +
			      e.getY() + ")");
	}
    }
	...
	if ( zoomPoint ) {
		...
		statusBar.setText( statusMessage + ": Zooming to (" + x1 + "," + y1 +")");
		zoom2Point(x1,y1);
		repaint();
	    } else if ( SwingUtilities.isRightMouseButton(e) ) {
		statusBar.setText( statusMessage + ": Zooming out from (" + x1 + "," + y1 +")");
		zoomOutFromPoint(x1,y1);
		repaint();
	    } else {
		statusBar.setText( "Left click zooms in. Right click zooms out." );
	    }
	}
    }
